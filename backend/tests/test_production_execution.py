...
def test_execution_requires_started_batch(client):
    headers = auth_headers(client)
    _location, ingredient, _output, batch = seed_batch(client, headers, "REQ")
    response = client.post(f"/api/v1/production-batches/{batch['id']}/execute", headers=headers, json={"actual_output_quantity": 20, "materials": [{"item_id": ingredient["id"], "actual_quantity": 10}]})
