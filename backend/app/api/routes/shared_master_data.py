...
def publish_master_data(payload: PublishMasterData, db: Session = Depends(get_db), user: User = Depends(require_permission("integrations.*"))):
    ...
    digest = snapshot_digest(workspace)
    published = []
    for destination in destinations:
        event = enqueue_event(
            ...
            payload={**workspace, "snapshot_sha256": digest},
        )
        published.append({"destination": destination, "event_id": event.id})
    add_audit(db, actor_user_id=user.id, action="master_data.published", entity_type="master_data", entity_id="canonical", details={"destinations": destinations, "snapshot_sha256": digest})
    db.commit()
    return {"published": published, "summary": workspace["summary"], "snapshot_sha256": digest}
