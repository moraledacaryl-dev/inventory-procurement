...
export default function Page(){
 ...
 async function cancel(){if(!detail)return;const reason=window.prompt("Cancellation reason");if(!reason)return;const materialDisposition=detail.batch.status==="in_progress"?window.prompt("What happened to materials already issued or prepared?"):null;if(detail.batch.status==="in_progress"&&!materialDisposition)return;await run(()=>api(`/production-batches/${detail.batch.id}/cancel`,{method:"POST",body:JSON.stringify({reason,material_disposition:materialDisposition})}),"Batch cancelled")}
 ...
 const missingRequired=detail.materials.some(line=>!line.optional&&!actuals[line.item_id]);
 return <AppShell title={detail.batch.batch_number} description={`${detail.recipe.code} v${detail.recipe.version} — ${detail.recipe.name}`}>
  ...
  <section className="card section-gap"><h2>Execution controls</h2><p className="section-copy">Start the batch before posting actual ingredient usage and finished output. Every required ingredient needs an explicit actual quantity. Waste is costed separately from good output.</p><div className="inline-form">{canStart?<button className="primary compact" disabled={busy} onClick={()=>run(()=>api(`/production-batches/${detail.batch.id}/start`,{method:"POST"}),"Batch started")}>Start batch</button>:null}{canCancel?<button className="secondary compact" disabled={busy} onClick={cancel}>Cancel batch</button>:null}{detail.batch.stock_document_id?<Link className="secondary compact" href={`/stock/documents/${detail.batch.stock_document_id}`}>Open stock document</Link>:null}</div></section>
  <section className="card section-gap"><h2>Actual material usage</h2><div className="table-wrap"><table><thead><tr><th>Ingredient</th><th>Planned</th><th>Unit cost</th><th>Planned cost</th><th>Actual used</th></tr></thead><tbody>{detail.materials.map(line=><tr key={line.item_id}><td>{item(line.item_id)}{line.optional?" (optional)":""}</td><td>{formatQuantity(line.planned_quantity)}</td><td>{formatMoney(line.unit_cost)}</td><td>{formatMoney(line.planned_cost)}</td><td><input type="number" min="0" step="0.000001" value={actuals[line.item_id]??""} disabled={!canExecute||busy} onChange={event=>setActuals(current=>({...current,[line.item_id]:event.target.value}))} placeholder={line.optional?"0 if unused":"Required"}/></td></tr>)}</tbody></table></div></section>
  <section className="card section-gap"><h2>Complete production</h2><div className="inline-form"><input type="number" min="0.0001" step="0.0001" value={actualOutput} disabled={!canExecute||busy} onChange={event=>setActualOutput(event.target.value)} placeholder="Good output"/><input type="number" min="0" step="0.0001" value={waste} disabled={!canExecute||busy} onChange={event=>setWaste(event.target.value)} placeholder="Rejected/waste output"/><input value={notes} disabled={!canExecute||busy} onChange={event=>setNotes(event.target.value)} placeholder="Execution notes"/><button className="primary compact" disabled={!canExecute||busy||Number(actualOutput)<=0||missingRequired} onClick={()=>run(()=>api(`/production-batches/${detail.batch.id}/execute`,{method:"POST",body:JSON.stringify({good_output_quantity:Number(actualOutput),output_waste_quantity:Number(waste||0),notes:notes||null,materials:detail.materials.map(line=>({item_id:line.item_id,actual_quantity:Number(actuals[line.item_id]||0)}))})}),"Production completed and stock posted")}>Complete and post</button></div></section>
 </AppShell>;
}
