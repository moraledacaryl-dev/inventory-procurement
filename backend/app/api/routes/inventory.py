...
from app.models.inventory import Category, CountLine, CountSession, Item, Location, StockBalance, StockDocument, StockMovement, UnitOfMeasure
from app.models.user import User
from app.schemas.inventory import *
from app.services.controls import add_audit, next_document_number
from app.services.inventory import InventoryError, issue_entries, post_document, receipt_entries, transfer_entries
...
def save(db, row):
    ...
    try:
        ...
        db.rollback(); conflict("Duplicate or invalid record")


def dimension(db: Session, dimension_id: str | None, expected_type: str | None = None):
    if not dimension_id:
        return None
    row = db.get(OperationalDimension, dimension_id)
    if not row or (expected_type and row.dimension_type != expected_type):
        raise HTTPException(422, f"Invalid {expected_type or 'classification'}")
    return row


def validate_item_classification(db: Session, data: dict):
    workspace = dimension(db, data.get("primary_workspace_id"), "workspace")
    item_type = dimension(db, data.get("item_type_id"), "item_type")
    if item_type:
        if not item_type.parent_id:
            raise HTTPException(422, "Item type is not connected to a record class")
        record_class = dimension(db, item_type.parent_id, "record_class")
        data["record_class_id"] = record_class.id
        if workspace and item_type.workspace_id and item_type.workspace_id != workspace.id:
            type_workspace = dimension(db, item_type.workspace_id, "workspace")
            if type_workspace.behavior_key != "shared":
                raise HTTPException(422, "Item type belongs to a different workspace")
    elif data.get("record_class_id"):
        dimension(db, data["record_class_id"], "record_class")
    if data.get("department_id"):
        department = dimension(db, data["department_id"])
        if department.dimension_type not in {"department", "outlet"}:
            raise HTTPException(422, "Invalid department or outlet")
    if data.get("cost_center_id"):
        dimension(db, data["cost_center_id"], "cost_center")
    if data.get("default_location_id") and not db.get(Location, data["default_location_id"]):
        raise HTTPException(422, "Default location not found")
    return workspace


def sync_primary_workspace(db: Session, item: Item):
    assignments = db.scalars(select(ItemWorkspaceAssignment).where(ItemWorkspaceAssignment.item_id == item.id)).all()
    by_workspace = {row.workspace_id: row for row in assignments}
    for row in assignments:
        row.is_primary = row.workspace_id == item.primary_workspace_id
    if item.primary_workspace_id and item.primary_workspace_id not in by_workspace:
        db.add(ItemWorkspaceAssignment(item_id=item.id, workspace_id=item.primary_workspace_id, is_primary=True))


@router.get("/categories", response_model=list[CategoryOut])
def list_categories(db: Session = Depends(get_db), _: User = Depends(require_permission("items.read"))):
    return db.scalars(select(Category).order_by(Category.sort_order, Category.name)).all()


@router.post("/categories", response_model=CategoryOut, status_code=201)
def create_category(p: CategoryCreate, db: Session = Depends(get_db), _: User = Depends(require_permission("items.*"))):
    if p.parent_id and not db.get(Category, p.parent_id):
        raise HTTPException(422, "Parent category not found")
    return save(db, Category(name=p.name.strip(), description=p.description, parent_id=p.parent_id, sort_order=p.sort_order))


@router.get("/units", response_model=list[UnitOut])
def list_units(db: Session = Depends(get_db), _: User = Depends(require_permission("items.read"))):
    return db.scalars(select(UnitOfMeasure).order_by(UnitOfMeasure.code)).all()


@router.post("/units", response_model=UnitOut, status_code=201)
def create_unit(p: UnitCreate, db: Session = Depends(get_db), _: User = Depends(require_permission("items.*"))):
    return save(db, UnitOfMeasure(code=p.code.upper().strip(), name=p.name.strip(), precision=p.precision))


@router.get("/locations", response_model=list[LocationOut])
def list_locations(db: Session = Depends(get_db), _: User = Depends(require_permission("locations.read"))):
    return db.scalars(select(Location).order_by(Location.code)).all()


@router.post("/locations", response_model=LocationOut, status_code=201)
def create_location(p: LocationCreate, db: Session = Depends(get_db), _: User = Depends(require_permission("locations.*"))):
    if p.parent_id and not db.get(Location, p.parent_id):
        raise HTTPException(422, "Parent location not found")
...
def list_items(
    ...
    if q:
        stmt = stmt.where((Item.sku.ilike(f"%{q}%")) | (Item.name.ilike(f"%{q}%")))
    if active is not None:
        stmt = stmt.where(Item.is_active == active)
    if workspace_id:
        stmt = stmt.where(Item.primary_workspace_id == workspace_id)
    if record_class_id:
        stmt = stmt.where(Item.record_class_id == record_class_id)
    if item_type_id:
...
def create_item(p: ItemCreate, db: Session = Depends(get_db), user: User = Depends(require_permission("items.*"))):
    if not db.get(Category, p.category_id) or not db.get(UnitOfMeasure, p.base_unit_id):
        raise HTTPException(422, "Category or unit not found")
    data = p.model_dump()
    validate_item_classification(db, data)
    data["sku"] = p.sku.upper().strip()
    data["name"] = p.name.strip()
    item = Item(**data)
    db.add(item)
    try:
        db.flush()
        sync_primary_workspace(db, item)
        add_audit(db, actor_user_id=user.id, action="item.created", entity_type="item", entity_id=item.id, details={"sku": item.sku, "workspace_id": item.primary_workspace_id, "item_type_id": item.item_type_id})
...
def update_item(item_id: str, p: ItemUpdate, db: Session = Depends(get_db), user: User = Depends(require_permission("items.*"))):
    ...
    if data.get("category_id") and not db.get(Category, data["category_id"]):
        raise HTTPException(422, "Category not found")
    if data.get("base_unit_id") and not db.get(UnitOfMeasure, data["base_unit_id"]):
        raise HTTPException(422, "Unit not found")
    merged = {
        "primary_workspace_id": data.get("primary_workspace_id", item.primary_workspace_id),
        "item_type_id": data.get("item_type_id", item.item_type_id),
        "record_class_id": data.get("record_class_id", item.record_class_id),
        "department_id": data.get("department_id", item.department_id),
        "cost_center_id": data.get("cost_center_id", item.cost_center_id),
        "default_location_id": data.get("default_location_id", item.default_location_id),
    }
    validate_item_classification(db, merged)
    for key in merged:
        if key in data or key == "record_class_id" and data.get("item_type_id"):
            data[key] = merged[key]
    if "name" in data:
    ...
    if data.get("is_active") is False:
        ...
        if Decimal(balance_total) != 0:
            conflict("Item with non-zero stock cannot be deactivated")
    changes = {key: {"from": str(getattr(item, key)), "to": str(value)} for key, value in data.items() if getattr(item, key) != value}
    for key, value in data.items():
        setattr(item, key, value)
    try:
        sync_primary_workspace(db, item)
        add_audit(db, actor_user_id=user.id, action="item.updated", entity_type="item", entity_id=item.id, details={"changes": changes})
        db.commit(); db.refresh(item); return item
    except IntegrityError:
        db.rollback(); conflict("Item update could not be saved")
...
def list_counts(db: Session = Depends(get_db), _: User = Depends(require_permission("counts.create"))):
    return db.scalars(select(CountSession).options(selectinload(CountSession.lines)).order_by(CountSession.created_at.desc())).unique().all()
