...
export default function Page(){
 ...
 return <AppShell title={detail.batch.batch_number} description={`${detail.recipe.code} v${detail.recipe.version} — ${detail.recipe.name}`}>
  ...
  <section className="grid section-gap"><div className="card"><h2>Planned output</h2><strong>{formatQuantity(detail.batch.planned_quantity)}</strong></div><div className="card"><h2>Planned batch cost</h2><strong>{formatCurrency(detail.planned_total_cost)}</strong></div><div className="card"><h2>Planned unit cost</h2><strong>{formatCurrency(detail.planned_cost_per_output_unit)}</strong></div></section>
  <section className="card section-gap"><h2>Execution controls</h2><p className="section-copy">Start the batch before posting actual ingredient usage and finished output. Completion posts one controlled stock document and records material, yield, and waste variances.</p><div className="inline-form">{canStart?<button className="primary compact" disabled={busy} onClick={()=>run(()=>api(`/production-batches/${detail.batch.id}/start`,{method:"POST"}),"Batch started")}>Start batch</button>:null}{canCancel?<button className="secondary compact" disabled={busy} onClick={()=>run(()=>api(`/production-batches/${detail.batch.id}/cancel`,{method:"POST"}),"Batch cancelled")}>Cancel batch</button>:null}{detail.batch.stock_document_id?<Link className="secondary compact" href={`/stock/documents/${detail.batch.stock_document_id}`}>Open stock document</Link>:null}</div></section>
  <section className="card section-gap"><h2>Actual material usage</h2><div className="table-wrap"><table><thead><tr><th>Ingredient</th><th>Planned</th><th>Unit cost</th><th>Planned cost</th><th>Actual used</th></tr></thead><tbody>{detail.materials.map(line=><tr key={line.item_id}><td>{item(line.item_id)}{line.optional?" (optional)":""}</td><td>{formatQuantity(line.planned_quantity)}</td><td>{formatCurrency(line.unit_cost)}</td><td>{formatCurrency(line.planned_cost)}</td><td><input type="number" min="0.000001" step="0.000001" value={actuals[line.item_id]||""} disabled={!canExecute||busy} onChange={event=>setActuals(current=>({...current,[line.item_id]:event.target.value}))}/></td></tr>)}</tbody></table></div></section>
  <section className="card section-gap"><h2>Complete production</h2><div className="inline-form"><input type="number" min="0.0001" step="0.0001" value={actualOutput} disabled={!canExecute||busy} onChange={event=>setActualOutput(event.target.value)} placeholder="Actual good output"/><input type="number" min="0" step="0.0001" value={waste} disabled={!canExecute||busy} onChange={event=>setWaste(event.target.value)} placeholder="Output waste"/><input value={notes} disabled={!canExecute||busy} onChange={event=>setNotes(event.target.value)} placeholder="Execution notes"/><button className="primary compact" disabled={!canExecute||busy||Number(actualOutput)<=0} onClick={()=>run(()=>api(`/production-batches/${detail.batch.id}/execute`,{method:"POST",body:JSON.stringify({actual_output_quantity:Number(actualOutput),output_waste_quantity:Number(waste||0),notes:notes||null,materials:detail.materials.map(line=>({item_id:line.item_id,actual_quantity:Number(actuals[line.item_id]||line.planned_quantity)}))})}),"Production completed and stock posted")}>Complete and post</button></div></section>
 </AppShell>;
}
