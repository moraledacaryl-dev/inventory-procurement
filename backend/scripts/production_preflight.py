...
def main():
    errors=[]; running_ci=os.getenv('CI','').lower()=='true'
    if settings.app_env=='production':
        ...
        if '*' in settings.trusted_hosts or ('testserver' in settings.trusted_hosts and not running_ci): errors.append('Production TRUSTED_HOSTS must contain only deployed hostnames')
