...
def test_pos_sale_consumption_idempotency_and_reversal(client):
    h=auth(client); service=pos_auth(); flour,sugar,_,loc,recipe=setup(client,h)
    ...
    sale=client.post('/api/v1/integrations/pos/events',headers=service,json=payload); assert sale.status_code==201
    duplicate=client.post('/api/v1/integrations/pos/events',headers=service,json=payload); assert duplicate.status_code==201 and duplicate.json()['id']==sale.json()['id']
    ...
    reversal=client.post('/api/v1/integrations/pos/events',headers=service,json={'external_event_id':'EVT-VOID-1','external_sale_id':'SALE-1','pos_system':'hidden-oasis-pos','event_type':'sale_voided','lines':[{'external_product_id':'MENU-CAKE','quantity':'2'}]}); assert reversal.status_code==201
    ...
    second=client.post('/api/v1/integrations/pos/events',headers=service,json={'external_event_id':'EVT-REFUND-1','external_sale_id':'SALE-1','pos_system':'hidden-oasis-pos','event_type':'sale_refunded','lines':[{'external_product_id':'MENU-CAKE','quantity':'2'}]}); assert second.status_code==409
