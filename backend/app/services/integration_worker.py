...
def accounting_envelope(event:IntegrationEvent)->dict:
    ...
    elif event.event_type=='procurement.purchase_order.approved':links={'purchase_order_id':payload.get('purchase_order_id'),'supplier_id':payload.get('supplier_id'),'commitment_total':payload.get('total')}
    elif event.event_type=='procurement.purchase_return.posted':links={'purchase_return_id':payload.get('purchase_return_id'),'purchase_order_id':payload.get('purchase_order_id'),'return_number':payload.get('return_number')}
    elif event.event_type=='inventory.production.completed':links={'batch_id':payload.get('batch_id'),'batch_number':payload.get('batch_number'),'stock_value':payload.get('total_cost'),'category':'Inventory production'}
    elif event.event_type in {'inventory.pos_sale_consumed','inventory.pos_sale_reversed'}:links={'sale_id':payload.get('sale_id'),'stock_document_id':payload.get('stock_document_id'),'category':'Cost of goods sold','reversal':event.event_type.endswith('reversed')}
