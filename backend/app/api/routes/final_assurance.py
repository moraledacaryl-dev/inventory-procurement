...
def snapshot(db:Session):
    ...
    controls=[('database','Database connectivity','passed',0),('stock_reconciliation','Stock ledger equals balances','passed' if not mismatches else 'failed',len(mismatches)),('negative_stock','No unauthorized negative stock','passed' if not negative else 'failed',len(negative)),('property_balances','Reusable-property balances are non-negative','passed' if not property_negative else 'failed',len(property_negative)),('hotel_pars','Hotel par quantities are valid','passed' if not invalid_pars else 'failed',len(invalid_pars)),('fixed_assets','Fixed-asset values and service status are valid','passed' if not asset_issues else 'failed',len(asset_issues)),('depreciation_periods','Depreciation periods are unique','passed' if not duplicate_periods else 'failed',len(duplicate_periods)),('posted_documents','Posted documents contain movements','passed' if not empty else 'failed',len(empty)),('item_classification','All active items have workspace, item type, and record class','passed' if not unclassified else 'attention',len(unclassified)),('integration_failures','No failed or dead-letter integrations','passed' if failed==0 and dead==0 else 'failed',failed+dead),('accounting_mapping','Accounting events are mapped','passed' if unmapped==0 else 'failed',unmapped),('backup','Completed backup is within freshness limit','passed' if backup_problem==0 else 'failed',backup_problem),('open_counts','No unfinished count sessions','passed' if open_counts==0 else 'attention',open_counts),('open_production','No unfinished production batches','passed' if open_production==0 else 'attention',open_production),('operational_requests','No unresolved Staff/Command Center requests','passed' if unresolved==0 else 'attention',unresolved)]
    ...
    return {'generated_at':now().isoformat(),'overall_status':'critical' if failed_checks else 'attention' if attention else 'healthy','summary':{'failed_checks':failed_checks,'attention_checks':attention,'stock_mismatches':len(mismatches),'negative_stock_violations':len(negative),'negative_property_balances':len(property_negative),'invalid_hotel_pars':len(invalid_pars),'fixed_asset_issues':len(asset_issues),'duplicate_depreciation_periods':len(duplicate_periods),'empty_posted_documents':len(empty),'unclassified_items':len(unclassified),'pending_integrations':pending,'failed_integrations':failed,'dead_letter_integrations':dead,'unmapped_accounting_events':unmapped,'unresolved_operational_requests':unresolved,'open_count_sessions':open_counts,'open_production_batches':open_production},'checks':checks,'stock_mismatches':mismatches[:200],'negative_stock':negative[:200],'negative_property_balances':property_negative[:200],'invalid_hotel_pars':invalid_pars[:200],'fixed_asset_issues':asset_issues[:200],'empty_documents':empty[:200],'unclassified_items':unclassified[:500],'latest_backup_at':backup.created_at.isoformat() if backup else None,'backup_age_hours':backup_age}
